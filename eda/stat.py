...
def twoMeans(total_df):
    ...
    apt_df = total_df[(total_df['HOUSE_TYPE'] == '아파트') & (total_df['month'].isin([11, 12]))]
    st.markdown("### 집계 \n"
                "- 11월과 12월의 아파트 가격을 비교한다.")
    ttest_df = round(apt_df.groupby('month')['OBJ_AMT'].agg(["mean", "std", "size"]), 1)
    st.dataframe(ttest_df, use_container_width=True)

    st.markdown("###  서울시 통합 11월 vs 12월 차이 검정\n"
                "- 11월과 12월의 아파트 평균 가격의 차이를 검정한다. \n"
                "- 가설설정 \n"
                "   + 귀무가설 : $H_{0}$: 11월과 12월의 아파트 평균 차이는 없다. \n"
                "   + 대립가설 : $H_{1}$: 11월과 12월의 아파트 평균 차이는 있다. \n")

    march_df = apt_df[apt_df['month'] == 11]
    april_df = apt_df[apt_df['month'] == 12]
    result = ttest(march_df['OBJ_AMT'], april_df['OBJ_AMT'], paired=False)
    st.dataframe(result, use_container_width=True)
    st.markdown(f"- 확인결과 p-value 값이 {result['p-val'].values[0]} 이므로 $H_{0}$을 채택하여, 11월과 12월의 아파트 평균 차이는 없다.")

    selected_sgg_nm = st.sidebar.selectbox("자치구명", sorted(total_df['SGG_NM'].unique()))
    st.markdown(f"### 서울시 {selected_sgg_nm} 11월 vs 12월 차이 검정\n"
                "- 자치구를 선택하여 11월과 12월의 아파트 평균 차이가 있는지 확인하도록 한다.")

    sgg_df = apt_df[apt_df['SGG_NM'] == selected_sgg_nm]
    sgg_march_df = sgg_df[sgg_df['month'] == 11]
    sgg_april_df = sgg_df[sgg_df['month'] == 12]
    ...
    if sgg_result['p-val'].values[0] > 0.05:
        st.markdown(f"- 확인결과 p-value 값이 {sgg_result['p-val'].values[0]} 이므로 $H_{0}$을 채택하여, 11월과 12월의 아파트 평균 차이는 없다.")
    else:
        st.markdown(f"- 확인결과 p-value 값이 {sgg_result['p-val'].values[0]} 이므로 $H_{1}$을 채택하여, 11월과 12월의 아파트 평균 차이는 있다.")

    st.markdown(f"### 서울시 :blue[{selected_sgg_nm}] 11월 vs 12월 시각화", unsafe_allow_html=True)
